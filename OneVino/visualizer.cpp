...
*/
    ...
    cv::putText(img,out.str(),cv::Point2f(static_cast<float>(f->_location.x), static_cast<float>(f->_location.y - 20)),cv::FONT_HERSHEY_SIMPLEX,0.6,cv::Scalar(255, 0, 0),1,8,false);
...
/*
    ...
    photoFrameVisualizer->draw(img, f->_location, cv::Scalar(255, 0, 0));
/*
    if (drawEmotionBar) {
